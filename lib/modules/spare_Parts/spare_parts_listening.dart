...
import 'package:mech_manager/components/spare_part_page/spare_part_list_row.dart';
...
class _SparePartsPageState extends State<SparePartsPage>
  ...
  void _fetchInitialSpareParts() {
    context.read<JobSheetBloc>().add(const FetchSparePartList(
  ...
  void _searchSparePart() {
    ...
    context.read<JobSheetBloc>().add(FetchSparePartList(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                        ...
                        isDense: true,
                        hintText: "Search by spare part name",
                        hintStyle:
              ...
              BlocConsumer<JobSheetBloc, JobSheetState>( listener: (context,state){},builder: (context,state) 
              {
                if (state.status == jobSheetStatus.initial ||
                          state.status == jobSheetStatus.loading) {
                        return const CenterLoader();
                      }
                      if (state.sparePartListing.isEmpty) {
                    return const Text('No record found');
                      ...
                      physics: const NeverScrollableScrollPhysics(),
                        itemBuilder: (context, index) {
                          return (index >= state.sparePartListing.length)
                              ? const CenterLoader()
                              : SparePartListRow(
                                  sparepartlist: state.sparePartListing[index],
                                );
                        },
                        itemCount: state.sparePartListing.length,
                      );
              },)
            ],
          ),
