...
import 'package:mech_manager/components/customer_page/customer_row_list.dart';
...
class _CustomerPageState extends State<CustomerPage>
  ...
  final ValueNotifier<String> activeRouteNotifier =
      ValueNotifier<String>('/customer_listing');
  Timer? _debounce;
  ...
  void _customerSearch() {
    final keyword = searchController.text;
    context.read<JobSheetBloc>().add(FetchCustomer(
  ...
  void _fetchInitialLabour() {
    context
        .read<JobSheetBloc>()
        .add(const FetchCustomer(status: jobSheetStatus.loading));
  }
          ...
          child: Column(
            ...
            children: [
              ...
              BlocConsumer<JobSheetBloc, JobSheetState>( listener: (context,state){},builder: (context,state){
                  ...
                  {
                    return (index>= state.customerListing.length)?const CenterLoader():CustomerRowList(customerModel: state.customerListing[index],);

                  },
                  itemCount: state.customerListing.length,
                  );
              },)
            ],
          ),
